...
CONVAR_CALLBACK_PROTO (ManiAdminBuildDate);
...
ConVar mani_admin_build_date ( "mani_admin_build_date", __DATE__, 0, "The date this binary was built.", CONVAR_CALLBACK_REF(ManiAdminBuildDate));
...
bool CAdminPlugin::AddBan ( player_t *player, const char *key, const char *initiator, int ban_time, const char *prefix, const char *reason ) {
	...
	if ( ban.byID ) { // ban by steamid
		...
		if ( reason ) {
			...
			engine->ServerCommand(ban_cmd);
 			LogCommand (player, "%s [%s] [%s] %s\n", localprefix, player->name, key, ban_cmd);
		}
	} else { // ban by IP
		...
		if ( reason ) {
			ban_cmd[len - 1] = 0;
			LogCommand (player, "%s [%s] [%s] %s (REASON: %s)\n", localprefix, player->name, player->ip_address, ban_cmd, reason);
		} else
			LogCommand (player, "%s [%s] [%s] %s", localprefix, player->name, player->ip_address, ban_cmd);
	}
...
bool MuteOptionsPage::PopulateMenuPage(player_t *player_ptr)
{
	...
	canmute = gpManiClient->HasAccess(player_ptr->index, ADMIN, ADMIN_MUTE, war_mode);
...
bool MutePlayerPage::PopulateMenuPage(player_t *player_ptr)
{
	...
	this->SetEscLink("%s", Translate(player_ptr, 506));

	if (strcmp(ban_type, "steam_id") == 0)
	{
		// ban by user id
		this->SetTitle("%s", Translate(player_ptr, 507));
	}
	else
	{
		// ban by ip
		this->SetTitle("%s", Translate(player_ptr, 508));
	}
	...
	for( int i = 1; i <= max_players; i++ )
	{
		...
		if (player_ptr->index != player.index &&
			gpManiClient->HasAccess(player.index, IMMUNITY, IMMUNITY_MUTE))
		{
			continue;
		}
...
int PlayerManagementItem::MenuItemFired(player_t *player_ptr, MenuPage *m_page_ptr)
{
	...
	{
		MENUPAGE_CREATE(MuteTypePage, player_ptr, 0, -1);
		return NEW_MENU;
	}
...
int MuteTypeItem::MenuItemFired(player_t *player_ptr, MenuPage *m_page_ptr)
{
	char *ban_type;
	...
	if (strcmp(ban_type, "steam_id") == 0)
	{
		MENUPAGE_CREATE_PARAM(MuteOptionsPage, player_ptr, AddParam("ban_type", ban_type), 0, -1);
		return NEW_MENU;
	}
	else if (strcmp(ban_type, "ip_address") == 0)
	{
...
bool MuteTypePage::PopulateMenuPage(player_t *player_ptr)
{
	this->SetEscLink("%s", Translate(player_ptr, 409));
	this->SetTitle("%s", Translate(player_ptr, 410));

	MenuItem *ptr;

	ptr = new MuteTypeItem;
	ptr->SetDisplayText("%s", Translate(player_ptr, 411));
	ptr->params.AddParam("ban_type", "steam_id");
	this->AddItem(ptr);

	ptr = new MuteTypeItem;
	ptr->SetDisplayText("%s", Translate(player_ptr, 412));
	ptr->params.AddParam("ban_type", "ip_address");
	this->AddItem(ptr);
...
CONVAR_CALLBACK_FN(ManiAdminBuildDate) {
	if (!FStrEq(pOldString, mani_admin_build_date.GetString()))
	{
		mani_admin_build_date.SetValue(__DATE__);
	}
}
