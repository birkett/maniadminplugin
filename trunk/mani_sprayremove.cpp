 ...
 )
{
	...
	if (argc - argv_offset == 3 && !FStrEq(menu_command, "spray"))
	{
		...
		if (FStrEq(menu_command, "spraywarn"))
		{
			...
			for (int i = 0; i < target_player_list_size; i++)
			{
				...
				admin_list[admin_index].flags[ALLOW_KICK])
		{
			// Warn target player about offending spray
			if (!FindTargetPlayers(admin_ptr, target, IMMUNITY_DONT_CARE))
			{
	...
	else
	{
		...
		if (spray_index == -1)
		{
			SayToPlayer(admin_ptr, "No spray tags within range of current position !!");
			return ;
		}
		...
		// No bans in lan mode !!!
		if (!lan_mode && admin_list[admin_index].flags[ALLOW_BAN])
		{
			AddToList((void **) &menu_list, sizeof(menu_t), &menu_list_size); 
...
)
{
	...
	if (!IsAdminAllowed(&player, "ma_spray", ALLOW_SPRAY_TAG, war_mode, &admin_index)) return PLUGIN_STOP;
